...
# Create bins.
def create_bins(df, dx=bin_angles[0], dy=2, outlier_thresh=10, min_thresh=100, tries=0):
    bins = {}
    max_x, max_y = 0, 0
    while max_x < 180:
        max_y = 0
        while max_y < math.ceil(df.loc[df[wind_features[1]].idxmax()][wind_features[1]]):
            ...
            if len(binned_df) >= outlier_thresh:
                if len(binned_df) < min_thresh:
                    return create_bins(df, dx=bin_angles[tries+1], dy=dy+2, outlier_thresh=outlier_thresh, min_thresh=min_thresh, tries=tries+1)
...
for outlier_thresh, min_thresh in bin_properties:
    ...
    bins, dx, dy, max_x, max_y = create_bins(df, outlier_thresh=outlier_thresh, min_thresh=min_thresh)
